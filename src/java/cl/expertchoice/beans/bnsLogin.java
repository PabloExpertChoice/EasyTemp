...
public class bnsLogin {
    ...
    /**
     * valida sesion
     * resibe los parametros siguientes parametros y consulta al base de datos si existe el usuario
     * @param user
     * @param pass
     * @return Uusuario()
     * @throws SQLException 
    ...
    public Usuario iniciarSesion(String user, String pass) throws SQLException {
        ...
        String sql = "SELECT nom_empresa, nom_user, user_emp\n"
                + "FROM CuentaEmpresa \n"
                + "WHERE user_emp = '" + user + "' \n"
        ...
        while (rs.next()) {
            usuario = new Usuario();
            usuario.setNom_empresa(rs.getString("nom_empresa"));
